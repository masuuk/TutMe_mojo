...
#  that explains each part). Run with:   mojo georef.mojo
# ============================================================================
...
def fit_affine(gcps: List[GCP]) -> Affine6:
    ...
    for i in range(len(gcps)):
        # index-based — GCPS iteration needs Copyable elements
        var gcol = gcps[i].col
        var grow = gcps[i].row
        var gX = gcps[i].X
        var gY = gcps[i].Y
        # X equation: [c, r, 1, 0, 0, 0 | X]
        var r1: List[Float64] = [gcol, grow, 1.0, 0.0, 0.0, 0.0, gX]
        # Y equation: [0, 0, 0, c, r, 1 | Y]
        var r2: List[Float64] = [0.0, 0.0, 0.0, gcol, grow, 1.0, gY]
        for p in range(6):
            for q in range(6):
